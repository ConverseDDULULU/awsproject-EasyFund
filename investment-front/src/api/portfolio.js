...
export async function getModelPortfolio(riskClass) {
  const res = await client.get(`/portfolio/model/${encodeURIComponent(riskClass)}`);
  return res.data; // { "달러": 10, "채권": 25, "주식": 55, "금": 10 } 이런 형태
}

// 과거 10년 수익률 & FX/금 시계열
export async function getPastReturns(riskClass) {
  const res = await client.get(`/portfolio/past/${encodeURIComponent(riskClass)}`);
  return res.data; 
  // { categories: [...], fx_gold: [...] }
}
...
export async function getFutureForecast(riskClass) {
  const res = await client.get(`/portfolio/forecast/${encodeURIComponent(riskClass)}`);
  return res.data; // { expected_return: 12.52 }
}

// (필요하면) 기대 수익률만 별도로 가져오는 API
export async function getExpectedReturn(riskClass) {
  const res = await client.get(`/portfolio/expected/${encodeURIComponent(riskClass)}`);
